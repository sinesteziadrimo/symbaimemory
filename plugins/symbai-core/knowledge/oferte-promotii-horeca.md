...
- **Interval orar (NOU — acum funcționează).** `create_offer` acceptă `timeStart`, `timeEnd` și `daysOfWeek` → oferta rulează DOAR în acea fereastră (Happy Hour). FĂRĂ aceste câmpuri oferta rulează 24/7 — atenție, nu lăsa un „-30% la prânz" să se aplice și la cină.
