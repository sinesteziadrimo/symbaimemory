...
3. **Cum afli pe care e clientul** — în ordinea asta:
   ...
   - Sau confirmă vizual: dacă în meniu vede **Panou Fabrică**, **Enterprise Readiness**, **Integrări/Date Master/Calitate/Randament/Validare/Rollout Enterprise**, **Execuție Producție**, **Planificare MPS** sau **Comenzi B2B** → e pe **fabrică**. Dacă vede doar pagina **Producție** (`/productie-evenimente`) → e pe **restaurant/simplu**.
...
Flux pe operații, execuție pe stații, planificare și trasabilitate fină. Pagini: **Fluxuri Tehnologice** (`/fluxuri-tehnologice`), **AI Flow Builder** (`/ai-flow-builder`), **Echipamente & Zone** (`/production/equipment-zones`), **Plan Fabrică 2D** (`/factory-floor-plan`), **Execuție Producție** (`/production`), **Tabletă Stație** (`/workstation-tablet`), **Scanner Containere** (`/production/scanner`), **Symbai Staff** (app mobila pentru operator: sarcini, operatii, containere QR), **Planificare MPS/MRP** (`/planificare-mps`, include wizard Smart Plan B2B), **Panou Fabrică** (`/factory-dashboard`), **Enterprise Readiness** (`/factory-enterprise-readiness`) si centrele `/factory-integrations`, `/factory-master-data`, `/factory-quality`, `/factory-yield-costing`, `/factory-validation`, `/factory-rollout`, **Loturi & WIP** (`/loturi-wip`), **Comenzi B2B** (`/b2b-orders`), **HACCP** (`/haccp`).
