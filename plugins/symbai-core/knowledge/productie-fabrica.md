...
Dacă userul întreabă „desenează-mi hala", „unde pun utilajele/magaziile pe plan", „vreau planul fabricii" sau „arată-mi fluxul fizic prin hală", folosește pagina **Plan Fabrică 2D** (`/factory-floor-plan`). Este un editor vizual dedicat fabricii, separat de Plan Sală: planul are nivele/etaje, obiecte cu geometrie în centimetri și conexiuni de tip flux material / conveyor / personal / utilități. Obiectele pot fi legate de entități reale: `production_equipment`, `production_zone`, `warehouse`, `storage_zone`; mutarea pe canvas schimbă doar geometria planului, nu modifică echipamentul/magazia reală.

Workflow agent:
1. Creează/verifică întâi entitățile reale prin MCP: `create_production_zone`, `create_production_equipment`, tool-urile de magazii/stocuri sau paginile de inventar existente. Planul nu înlocuiește configurarea reală.
2. Pentru desen/aranjare folosește browserul (skill Chrome/browser): `gaseste_in_aplicatie("plan fabrica 2D")` sau deschide `/factory-floor-plan`, apoi fă screenshot după ce ai pus/aranjat obiectele. MCP nu are încă tool dedicat pentru mutarea coordonatelor de canvas.
3. Pe plan apar live statusul echipamentului și stocul sumar pe zonele de depozitare; pentru cifre decizionale verifică prin tool-uri de stoc/producție, nu doar vizual.
4. Explică userului simplu: „Am pus cuptorul, zona de ambalare și magazia pe plan; culorile HACCP/temperatura sunt metadate vizuale, iar stocul real se vede pe zonele de depozitare."
...
| „Desenează/aranjează hala fabricii" | Creezi/verifici întâi zonele, echipamentele, magaziile și zonele de depozitare prin MCP; apoi deschizi `/factory-floor-plan` cu browserul, plasezi obiectele pe Plan Fabrică 2D și arăți screenshot. |
