...
2. **Context (citiri, fă-le întâi):** `list_brands` (+ `list_locations`) → `brandId`. `list_websites(brandId)` → există deja un site? ce `id`/`kind`? `list_website_component_catalog(includeSchema:true)` → ce componente/config-uri ai la dispoziție (inclusiv `custom-html`). `get_ecommerce_settings(brandId)` → monedă/TVA/checkout. `list_menus`/`list_menu_items` → are catalog cu prețuri? `list_menu_categories` → câte categorii, au ierarhie? `browse_brand_media(brandId)` → poze pentru hero/logo.
...
3. **Replică** prin tool-urile MCP, mapând `sourceBrief.sections` pe `set_hero` / `add_website_section` / `upsert_custom_website_component`, `sourceBrief.navigation` pe `update_website_navigation`, `sourceBrief.branding` pe temă/logo, iar footerul pe `set_website_footer`. Pentru catalog: produse → categorii → filtre → PDP → legale.
