...
| „Operațiile pentru produsul Y / fă-mi fluxul" | Doar **fabrică** → citește fluxul cu `get_flow_graph` dacă există, apoi editează DOAR semantica (`build_complete_flow`, `add_flow_operation`, `add_operation_dependency`, materiale/output-uri/QC). Dependențe: FS = pornește după finalizarea predecesorului; SS = pornește după ce predecesorul a pornit (paralel/overlap); FF/SF sunt pentru finalizare. După editare: `auto_arrange_diagram(flowVersionId)` → `validate_flow_consistency`. Nu seta coordonate x/y. |
...
| „Am greșit / reversează operația" | `exec_correct_quantities` cu `correctionType:"reversal"` și `reason`. Dacă tool-ul întoarce eroare, operația nu e reversată și se poate relua după remedierea cauzei. Dacă răspunsul e `success:true`, verifică `reversalDocId` și `outputReversalDocId`, apoi confirmă prin citire în stoc; dacă un ID lipsește, stornarea de stoc trebuie verificată înainte de retry. |
