...
- **POS Ospătar** `/pos/waiter` — Interfața POS a ospătarului (= POSMobile cu channelId=waiter): plan sală cu mese, bonul mesei, meniu adăugare, plată, operațiuni masă, rezervări, comenzi fără masă (guest). Pe tabletă apare workspace lateral.
...
- **POS Bar** `/pos/bar` — Aceeași interfață POSMobile pe canalul „bar" (channelId=bar): comandă per masă, meniu, plată, operațiuni. Pe desktop are coloană coș + suport scanner coduri bare. Meniul prioritar bar se setează prin profil afișaj.
  · NU aici: pentru ospătar servire la masă → /pos/waiter; recepție hotel → /pos/reception
- **POS Mobil** `/pos/mobile` — Interfața principală POS a ospătarului (PWA, channelId=mobile): ecrane plan sală (mese), produse masă (bon nefiscal), meniu adăugare, plată, operațiuni masă (transfer/reducere/retur/split/partajare), rezervări, comandă guest fără masă.
...
- **Plan Sală (Editor)** `/plan-sala` — DESENEZI/editezi planul: mese, zone, pereți, decor, configurații de sală (designer).
...
- **Meniu Fizic** `/menu?tab=physical-menu` — Designer de meniu tipărit/PDF (lazy-loaded PhysicalMenuDesigner): formate A4/A3, paginare „Recalculează”, copertă + pagini nutriționale/alergeni, export PDF/print, QR dinamic pe copertă.
...
- **Stoc Curent (Live)** `/inventory-check?tab=live-stock` — Cantități + valoare pe gestiuni în timp real, căutare/filtrare, alerte stoc minim.
...
- **Sold Furnizori** `/supplier-balances` — Facturi furnizori cu solduri și plăți restante: tabel facturi (achitate/parțial/scadente/depășite), carduri totaluri, înregistrare plată (dialog Data Plății), filtre+căutare. Wrapper peste componenta AccountsPayable.
...
- **Raport Zilnic (Z)** `/analytics?tab=daily` — Tab Raport Zilnic — sumar vânzări pe zi în stil raport Z (buton printare, detaliere TVA pe cote), delegat la componenta DailyReportView pe intervalul/brand/locația selectate.
...
- **P&L Detaliat** `/analytics?tab=pnl` — Tab P&L (Profit & Loss) detaliat pe categorii cu drill-down — delegat la componenta DetailedPnL pe perioada/brand/locația selectate.
  · NU aici: pentru P&L cu export/snapshot și filtre proprii → /reports/pnl; pentru KPI cu praguri → /reports/pnl-kpi
- **Vânzări Angajați** `/analytics?tab=staff_sales` — Tab Vânzări Angajați — performanță vânzări per angajat (componenta SalesByStaff), pe interval/brand/locație.
...
- **Analiză Mese** `/analytics?tab=tables` — Tab Analiză Mese — ocupare, rotație, venit per masă (componenta TablePerformance), pe interval/brand/locație.
  · NU aici: pentru rotație/ocupare din perspectivă rezervări → /reservations?tab=analytics; pentru editarea layoutului → /floorplan
- **Timpi Așteptare** `/analytics?tab=speed` — Tab Timpi Așteptare — analiza timpilor de servire/bucătărie/livrare (componenta SpeedOfService), pe interval/brand/locație.
...
- **P&L KPI** `/reports/pnl-kpi` — Pagină KPI P&L — afișează indicatorii definiți pe categorii (venituri, marketing, leaduri, evenimente, finanțe, operațional) cu valoare live, semafor și trend (componenta PnLKpiTab). Definițiile/pragurile se editează din Setări > Setări P&L.
...
- **Foaie Pontaj** `/staff?tab=timesheets` — Tabel cu pontaje efective (TimeEntry): intrare/ieșire, pauze, total ore, ore suplimentare, cu căutare, filtru brand/perioadă, aprobare și export/print. Realitatea lucrată, nu planul.
