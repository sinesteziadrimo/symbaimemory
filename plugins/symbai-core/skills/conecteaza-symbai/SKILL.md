...
## Configurarea corectă (încearcă în ordine)

1. **Terminal cu CLI** — dacă comanda `claude` există în terminal (verifică cu `claude --version` prin Bash):
   ```
...
2. **Fără CLI** (tipic: aplicația Claude Desktop, tab-ul Code) — editează direct fișierul `.claude.json` din folderul home al utilizatorului (Windows: `C:\Users\<nume>\.claude.json`; macOS/Linux: `~/.claude.json`). În obiectul `"mcpServers"` de la **nivelul rădăcină** al JSON-ului (creează-l dacă lipsește) adaugă:
   ```json
   "symbai": { "type": "http", "url": "<URL>", "headers": { "Authorization": "Bearer <TOKEN>" } }
   ```
   Păstrează restul fișierului neatins. Imediat după salvare, cere-i utilizatorului să închidă complet și să redeschidă aplicația — configurația se citește la pornire, iar aplicația poate suprascrie fișierul dacă sesiunea continuă mult după editare.

## Capcana #1 — claude_desktop_config.json (NU-l folosi)

`claude_desktop_config.json` (Windows: `%APPDATA%\Claude\`; macOS: `~/Library/Application Support/Claude/`) este fișierul aplicației Claude **Desktop** (tab-ul Chat) și acceptă DOAR servere locale stdio (`command` + `args`). O intrare HTTP `symbai` pusă acolo **nu funcționează** și produce la pornirea aplicației eroarea:

> **"Some MCP servers could not be loaded ... are not valid MCP server configurations and were skipped: symbai"**

Dacă utilizatorul vede eroarea asta: deschide fișierul, șterge intrarea `symbai` din `mcpServers` (doar intrarea, nu restul fișierului), salvează. Apoi fă configurarea corectă de mai sus.
...
3. **După restart**, într-o sesiune NOUĂ: `/mcp` trebuie să arate serverul `symbai` conectat, iar tool-urile Symbai devin disponibile.
