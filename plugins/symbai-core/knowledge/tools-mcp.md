...
- `analyze_recipes` — Analizează rețetele unui brand (completitudine, costuri, ingrediente lipsă); include rețetele brandului + rețetele globale (`brandId` gol), nu tot tenantul. (necesită: brandId)
...
- `list_recipes` — Listează rețetele (semi-preparate, preparate finite) din sistem; cu `brandId` filtrează pe brand + rețete globale. (parametri opționali: productType, query, brandId, limit)
...
- `get_defect_pareto` — Analiză Pareto a defectelor: tipurile de defecte cele mai frecvente, cantități respinse, procent din total; normalizează rezultatele QC vechi/noi și nu numără `passed` ca defect. (parametri opționali: days)
...
- `forecast_production_demand` — Previziune cerere de producție make-to-stock din istoricul POS + B2B: cerere săptămânală, tendință, variabilitate, stoc de siguranță, `forecastHorizonTotal` brut, comenzi ferme deschise `firmOpenOnHorizon` (`confirmed`/`in_production`/`picking`/`packed`, nu `dispatched`/`delivered`) și `netForecastHorizon` pentru producția pe stoc fără dublă-numărare. (parametri opționali: productIds, lookbackWeeks, horizonWeeks, method, channel)
...
- `get_qc_stats` — Obține statistici QC: rata de pass/fail, tip defecte frecvente, inspecții pe perioadă; normalizează `pass/passed/ok` și `fail/failed/rejected` înainte de calcul. (parametri opționali: days)
...
- `add_operation_dependency` — Adaugă o dependență între două operații: FS (Finish-Start), SS (Start-Start), FF (Finish-Finish), SF (Start-Finish) cu lag time. FS/SS sunt impuse la pornirea operației; folosește SS cu lag 0 pentru operații paralele. (necesită: flowVersionId, fromOperationId, toOperationId)
...
- `exec_correct_quantities` — Corectează cantitățile declarate anterior pe o execuție de operație (ajustare sau reversare completă). La `reversal`, eroarea înseamnă că operația nu e marcată reversată și retry-ul refolosește documentele de storno existente; la `success:true`, stornările aplicabile sunt postate și verifici `reversalDocId`/`outputReversalDocId` prin citire în stoc. (necesită: operationExecutionId, correctionType, reason)
...
- `exec_declare_consumption` — Declară consumul de materii prime pentru o operație în curs. Respectă FEFO; MCP nu poate forța abaterea doar cu `overrideReason`, deci la `fefo_violation` alege lotul corect sau folosește UI cu manager/QA autorizat. (necesită: operationExecutionId, items)
