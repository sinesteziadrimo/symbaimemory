...
3. **Repară pe categorii**, în ordinea impactului din raport:
   ...
   - Produse fără slug → `bulk_optimize_product_seo` le pune slug-uri ASCII.
   - Pagini de categorie generice (titlu „Categorie — Magazin", fără text) → `set_category_seo` (titlu + meta + text intro/outro + slug per categorie). Vezi `pagini-seo-2026.md` → secțiunea PLP.
