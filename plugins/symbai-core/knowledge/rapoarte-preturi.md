...
- **P&L Livrări** (`/reports/pnl-livrari`) — configurezi segmente de livrare: brand/locație de venit, angajați desemnați pentru manoperă și reguli de cheltuieli (fix, procent din CA, 100% sau procent dintr-o categorie reală). În chat folosești `list_delivery_pnl_segments` → `get_delivery_pnl`.
...
2b. **Verific produse pe pierdere / profit pe SKU** — `get_product_pnl(mode:"loss", perioada, limit)` → repari costuri lipsă, rețete/prețuri sau ponderea de alocare; folosește `warnings` și `reconciliation` înainte să tragi concluzii.
2c. **Verific livrările separat** — `list_delivery_pnl_segments` → `get_delivery_pnl(configId, perioada)`. Dacă nu există segment salvat, `get_delivery_pnl(brandId)` e doar calcul ad-hoc venit−marfă; pentru manoperă + cheltuieli configurate deschide `/reports/pnl-livrari`.
