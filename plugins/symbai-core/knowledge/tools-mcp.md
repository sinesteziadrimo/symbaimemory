...
- `exec_correct_quantities` — Corectează cantitățile declarate anterior pe o execuție de operație (ajustare sau reversare completă). La `reversal`, eroarea înseamnă că operația nu e marcată reversată; la `success:true`, verifică `reversalDocId` și `outputReversalDocId`, iar lipsa unuia cere verificare de stoc înainte de retry. (necesită: operationExecutionId, correctionType, reason)
