...
## Reguli de aur
- **Context întâi:** `list_brands` + `list_locations`, apoi `list_cash_registers` (registerId-ul casieriei). Aproape toate tool-urile cer registerId / brandId / locationId. ID-uri, nu nume.
...
5. **Pasul 3 — sigilare.** Doar după ce verificările sunt curate. Arată-i userului ce se închide, cere OK, apoi `close_cash_book_day(registerId, businessDate, confirm: true)` SAU îl lași să apese „Închide ziua" în wizard. Ziua primește filă + sigiliu SHA-256 (lanț cu ziua precedentă) și devine „Închisă/Sigilată". Foaia completă de închidere (PDF) se descarcă tot din Pasul 3.
6. **Plafon depășit (RO 50.000 lei/zi sau 10.000 lei/partener).** Dacă soldul e peste limită, sistemul avertizează (și depășirea per partener BLOCHează închiderea). Fă o depunere la bancă: arată suma → cere OK → `create_cash_book_entry(registerId, entryType: "bank_deposit", amount, description, confirm: true)`, sau trimite userul la `/finance/cash-book` → „Depunere Bancă". Reverifică cu `get_cash_register_balance`.
7. **Cheltuială mică cash în timpul zilei** (apă, reparație): arată suma → cere OK → `create_cash_book_entry(registerId, entryType: "payment", amount, description, confirm: true)`, sau în `/finance/cash-book` → „Plată (DPÎ)". Pentru evidența P&L a unei cheltuieli generale: `create_expense(brandId, category, amount)`.
...
- Scriere `financiar` (cer OK explicit, sigilarea e definitivă): `close_cash_book_day` 🔒, `create_cash_book_entry`, `void_cash_book_entry`, `transfer_between_cash_registers`, `create_expense`. Scriere `inventar`: `generate_daily_consumption`.
