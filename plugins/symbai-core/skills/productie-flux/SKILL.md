...
- **QC hold / carantină / CAPA** — un lot poate fi blocat la calitate cu motiv: `create_quality_hold` (necesită `lotId`, `eventType`); cât e blocat, containerele lui nu pot avansa/împărți/uni. La recepția materiei prime folosește front-door HACCP: `list_quarantine_lots` pentru coadă și `record_incoming_inspection(lotId, decision)` pentru accept / carantină / respingere, după ce confirmi lotul și decizia. Pentru neconformități recurente, audit sau reclamații, folosește CAPA/NCR: `open_capa` → `update_capa` pe statusurile investigating/action/verification/closed; citești cu `list_capa`. Eliberezi hold-ul cu `release_quality_hold` (necesită `holdId`, `releasedBy`). Statusul: `exec_get_lot_qc_status` / `list_quality_holds`.
