...
6. **Corecții**: `exec_correct_quantities` (`operationExecutionId`, `correctionType` = qty_adjustment / reversal, `reason` obligatoriu; opțional `qtyGoodDelta`, `qtyScrapDelta`, `qtyReworkDelta`, `employeeId`). La `reversal`, MCP creează storno pentru consum (`reversalDocId`) și pentru output (`outputReversalDocId`) ca să nu rămână stoc fantomă. Dacă tool-ul întoarce eroare la citire/creare document sau linie, operația NU e marcată reversată și rămâne retryable după corectarea cauzei. Dacă răspunsul e `success:true`, dar un ID lipsește, înseamnă că doar postarea acelui storno trebuie verificată în aplicație/registrul de stoc; nu repeta orbește aceeași corecție.
...
- **Dependențe**: `add_operation_dependency` (`flowVersionId`, `fromOperationId`, `toOperationId`, tip FS/SS/FF/SF); `remove_operation_dependency` (`dependencyId`). FS blochează pornirea operației până când predecesorul e finalizat; SS blochează pornirea până când predecesorul a pornit (pentru operații paralele/overlap). FF/SF sunt constrângeri de finalizare, nu de start. Pentru operații simultane, folosește SS cu `lagMinMinutes:0`, nu lăsa o ordine presupusă să pară dependență reală.
