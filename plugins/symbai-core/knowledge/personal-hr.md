...
- Sarcini: `create_task_list`/`create_task`/`bulk_create_tasks` sunt versiunile de BAZĂ (simple, deja disponibile). **După deploy-ul nexuspos** apar tool-uri NOI pentru modelul cu țintă/recurență/dovadă: `create_targeted_task_list` + `create_targeted_task` (scriere — ACESTEA, nu cele de bază, suportă targetRoleId/targetShift/targetSection + recurență + dovadă), plus `list_task_lists`/`list_tasks`/`get_task`/`get_task_dashboard`/`get_my_tasks` (citire) și `update_task_list`/`clone_task_list`/`update_task`/`complete_task`/`assign_task`. Până atunci, modelul complet (țintă, recurență, dovadă) se face din interfață. Vezi `knowledge/tasks-sarcini.md` + skill-ul `gestioneaza-sarcini`.
